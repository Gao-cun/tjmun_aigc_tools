...
export function Timeline({ documents }: { documents: HistoryDocument[] }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Historical Style Timeline</CardTitle>
      </CardHeader>
      <div className="space-y-3">
        {documents.length === 0 ? <p className="text-sm text-muted">No history documents yet.</p> : null}
        {documents.map((doc) => (
          <div key={doc.id} className="flex items-center gap-3 rounded-md border border-border bg-panelSoft p-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-muted">{doc.documentType} · {doc.meeting ?? "No meeting"} · {doc.documentDate ?? "No date"}</div>
            </div>
